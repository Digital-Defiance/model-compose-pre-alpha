...
def lock_gpu() -> None:
    r.set(GPU_LOCK_KEY, LOCKED)
    logger.debug("Acquired GPU LOCK.")

def unlock_gpu() -> None:
    r.set(GPU_LOCK_KEY, UNLOCKED)
    logger.debug("Released GPU LOCK.")
    
def gpu_is_locked() -> bool:
    return r.get(GPU_LOCK_KEY) == LOCKED
...
def redis_lock():
    ...
    logger.error("GPU was not released in time.")
...
enc = Encoder(encoder=encoder, bpe_merges=bpe_merges)
